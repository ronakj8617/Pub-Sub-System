...
int main() {
    ...
    CROW_ROUTE(app, "/getSubscriberCount").methods("GET"_method, "OPTIONS"_method)(
        [&manager](const crow::request &req) {
            auto urlParams = crow::query_string(req.url);
            std::string topicId = urlParams.get("topicId") ? urlParams.get("topicId") : "";
            return crow::response(200, json(manager.getSubscriberCount(topicId)));
        });
